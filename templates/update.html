...
def admin():
    ...
    for p in posts:
        if p.video_public_id:
            p.thumbnail_url = get_safe_cloudinary_video_thumbnail(p.video_public_id)
        else:
            p.thumbnail_url = get_safe_cloudinary_url(p.image_public_id)
